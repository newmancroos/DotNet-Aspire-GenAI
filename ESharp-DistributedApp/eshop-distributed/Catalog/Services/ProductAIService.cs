...
public class ProductAIService(IChatClient chatClient)
{
    public async Task<string> SupportAsync(string query)
    {
        ...
        //var resultPrompt = await chatClient.CompleteAsync(chatHistory);  // No CompleteAsync due to Deprecation
        var resultPrompt = chatClient.GetStreamingResponseAsync(chatHistory);

        var responseString = string.Empty;
        await foreach (var update in resultPrompt)
        {
